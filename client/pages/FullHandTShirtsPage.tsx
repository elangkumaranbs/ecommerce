...
import { getProductColors, getProductImage } from '../lib/utils';
...
const FullHandTShirtsPage = () => {
  ...
  const fullHandTShirts = useMemo(() => {
    ...
    return products.filter((product: Product) => {
      ...
      console.log('🔍 Checking product:', {
        ...
        matches: subcategory === 'full hand' || 
                 subcategory.includes('full hand') ||
                 subcategory.includes('full sleeve') ||
                 name.includes('full hand') ||
                 name.includes('full sleeve')
      });
      
      return (
        // Match by subcategory (most reliable)
        ...
        subcategory.includes('full hand') ||
        subcategory.includes('full sleeve') ||
        // Match by product name as fallback
        ...
        // Match by category if needed
        (category.includes('t-shirt') && (name.includes('full hand') || name.includes('full sleeve')))
      );
    });
  }, [products]);
  ...
  // Debug logging
  React.useEffect(() => {
    console.log('🛍️ FullHandTShirtsPage: All products loaded:', products?.length || 0);
    console.log('🛍️ FullHandTShirtsPage: Full Hand T-Shirts found:', fullHandTShirts.length);
    
    if (products && products.length > 0) {
      console.log('🛍️ All products with their categories and subcategories:');
      products.forEach((p, index) => {
        console.log(`${index + 1}. "${p.name}" | Category: "${p.category?.name}" | Subcategory: "${p.subcategory}"`);
      });
      
      console.log('🛍️ Unique subcategories in database:', 
        [...new Set(products.map(p => p.subcategory).filter(Boolean))]
      );
      
      if (fullHandTShirts.length === 0) {
        console.log('⚠️ No Full Hand T-Shirts found with current filter. Products that contain "hand" or "sleeve":');
        const potentialMatches = products.filter(p => {
          const name = p.name?.toLowerCase() || '';
          const subcategory = p.subcategory?.toLowerCase() || '';
          return name.includes('hand') || name.includes('sleeve') || subcategory.includes('hand') || subcategory.includes('sleeve');
        });
        console.log('Potential matches:', potentialMatches.map(p => ({
          name: p.name,
